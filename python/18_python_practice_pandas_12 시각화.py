# -*- coding: utf-8 -*-
"""
Created on Wed Dec 29 14:16:25 2021
...
# ------------------------------------------------------------------
# 4. hist : 히스토그램 (밀도 표현, 전체 합이 1)

s1 = Series(np.random.rand(1000)) # 정해진 숫자에서 무작위 추출(균등하게 -->> uniform dist)
s1.hist()

s1 = Series(np.random.randn(1000)) # 정규분포에서 무작위 추출
s1.hist()
si.hist(bins=4) # 막대의 개수 또는 계급의 구간
